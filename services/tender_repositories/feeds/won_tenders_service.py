...
class WonTendersService(BaseFeedService):
    ...
    def _build_won_query(
        ...
        # Для 44ФЗ: status_id = 3 (Разыграна)
        # Для 223ФЗ: используем старый способ (нет статуса "Разыграна" для 223ФЗ)
        if registry_type == "44fz":
            ...
            # Для 223ФЗ пока используем старый способ
            today = date.today()
    ...
    def _build_won_count_query(
        ...
        if registry_type == "44fz":
            won_filter, won_params = TenderQueryBuilder.build_won_tenders_filter(date.today(), use_status=True)
            query = select + won_filter
            params.extend(won_params)
        else:
            # Для 223ФЗ пока используем старый способ
            query = select + " AND r.end_date < %s"
            params.append(today)
            query += " AND r.delivery_end_date IS NOT NULL AND r.delivery_end_date >= %s"
            params.append(min_delivery_date)
            # Исключаем "Плохие" для 223ФЗ
