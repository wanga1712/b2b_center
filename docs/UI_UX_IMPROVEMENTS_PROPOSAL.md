...
**Текущий шаг:** Этап 3 — Детальная карточка сделки (CRM-summary и вкладки)
...
- [x] Визуально усилить `DealCard` (структура, типографика, фон, аккуратная сетка)
...
- [x] Создать таблицу `deal_chat` в БД для хранения сообщений
- [x] Реализовать `DealChatService` для работы с чатом
- [x] Реализовать `DealChatWidget` с выбором собеседника
- [ ] Интегрировать AI-агента (шаблонный) для общения в чате сделки
